...
to setup-turtles
  create-turtles number-people
    [ setxy random-xcor random-ycor
      ...
      set size 1.5
      get-healthy ]
  ask n-of number-of-infectors turtles
    [ get-sick ]
  if vaccine [ask n-of number-of-vaccinated turtles
    [ get-vaccinated ]]
  if masks [ ask n-of mask-wearers turtles
    [ wear-mask ] ]
end
...
to go
  ask turtles [
    ...
    ifelse sick? [ ifelse mask? [if random-float 10 > 100 [infect] ] [infect] ] [ reproduce ]
  ]
  if vaccine [ ask n-of 1 turtles
    [ get-vaccinated ] ]
  if masks [ ask n-of 1 turtles
    [ wear-mask] ]
  update-global-variables
  update-display
  tick
...
to infect
  ask other turtles-here with [ not sick? and not immune? ]
    [ifelse mask? [ if random-float 100 < infectiousness and random-float 10 > 100
      [ get-sick ] ] [ if random-float 100 < infectiousness
      [ get-sick ] ]]
end
...
to reproduce
  if count turtles < population-density and random-float 100 < chance-reproduce
    [ hatch 1
      [ set age 1
        lt 45 fd 1
        get-healthy ] ]
...
number-of-vaccinated
0
number-people - number-of-infectors
15.0
1
1
NIL
